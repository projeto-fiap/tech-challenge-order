...
public class CreateOrUpdateOrderUseCaseImpl implements CreateOrUpdateOrderUseCase {
	...
	public CreateOrUpdateOrderUseCaseImpl(OrderDataProvider orderDataProvider, RestTemplate restTemplate,
			String personUrl, InitializeItemUseCase initializeItemUseCaseImpl,
			CalculateTotalOrderUseCase calculateTotalOrderUseCase, String username, String systemPerson) {
	...
	private PersonDTO retrievePerson(PersonDTO person) {
		...
		String personServiceUrl = personUrl + initializePersonServiceUrl + "?cpf="
				+ person.getDocument().stream().findFirst().orElseThrow().getValue();
		ResponseEntity<List<PersonDTO>> response = restTemplate.exchange(personServiceUrl, HttpMethod.GET, entity,
