...
class CreateQrCodeUseCaseImplTest {
    ...
    void setUp() {
        ...
        createQrCodeUseCase = new CreateQrCodeUseCaseImpl(
                restTemplatePayments, restTemplateKeycloak, paymentBaseUrl, keycloakBaseUrl, clientId, clientSecret
        );
    }
    @Test
    void testExecute() throws IOException {
        ...
        when(restTemplatePayments.exchange(
                eq(paymentBaseUrl + "/api/v1/payments"),
                eq(HttpMethod.POST),
                any(HttpEntity.class),
                eq(byte[].class)
        ...
        verify(restTemplatePayments, times(1)).exchange(
                eq(paymentBaseUrl + "/api/v1/payments"),
                eq(HttpMethod.POST),
                any(HttpEntity.class),
                eq(byte[].class)
