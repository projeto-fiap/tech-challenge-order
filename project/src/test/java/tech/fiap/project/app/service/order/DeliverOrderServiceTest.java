...
class DeliverOrderServiceTest {
    ...
    void testExecute_OrderFoundAndDelivered() {
        ...
        when(restTemplate.exchange(eq("http://kitchen-service/api/kitchen/" + orderId), eq(HttpMethod.GET), any(HttpEntity.class), eq(KitchenDTO.class)))
                .thenReturn(new ResponseEntity<>(kitchenDTO, HttpStatus.OK));
        when(deliverOrderUseCase.execute(orderId)).thenReturn(deliveredOrder);
        orderMapperMockedStatic.when(() -> OrderMapper.toDTO(order, Optional.of(kitchenDTO))).thenReturn(orderResponseDTO);
        ...
        verify(restTemplate, times(1)).exchange(eq("http://kitchen-service/api/kitchen/" + orderId), eq(HttpMethod.GET), any(HttpEntity.class), eq(KitchenDTO.class));
...
//        verify(retrieveOrderUseCase, times(1)).findByIdWithPayment(orderId);
//        verify(restTemplate, never()).exchange(anyString(), any(HttpMethod.class), any(HttpEntity.class), eq(KitchenDTO.class));
...
//        when(restTemplate.exchange(eq("http://kitchen-service/api/kitchen/" + orderId), eq(HttpMethod.GET), any(HttpEntity.class), eq(KitchenDTO.class)))
//                .thenReturn(new ResponseEntity<>(kitchenDTO, HttpStatus.OK));
//        orderMapperMockedStatic.when(() -> OrderMapper.toDTO(order, Optional.of(kitchenDTO))).thenReturn(new OrderResponseDTO());
//
